...
export const getOrderStatus = async (req, res) => {
  try {
    ...
    const uAccount = await User.findById(user);
    const reffereduser = await User.findById(uAccount.referredBy);
    if (!orderId) {
    ...
    if (response.data.order_status == "PAID") {
      ...
      reffereduser.successfulReferrals += 1;
      reffereduser.walletBalance += 100; // Referral bonus
      await reffereduser.save();
      await Notification.create({
        userId: reffereduser._id,
        type: "referral",
        message: `Congrats! You've earned ₹100 in your wallet. ${uAccount.email} has successfully paid for a ticket.`,
      });
